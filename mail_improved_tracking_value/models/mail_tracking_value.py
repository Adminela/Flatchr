...
class MailTrackingValue(models.Model):
    ...
    def create_tracking_values(self, initial_value, new_value, col_name, col_info, track_sequence, model_name):
        ...
        if col_info['type'] in ('many2many', 'one2many'):
            ...
            #raise ValidationError("HELLO %s" %values)
            return values
        else:
            #values = super().create_tracking_values(initial_value, new_value, col_name, col_info, track_sequence, model_name)
            #raise ValidationError("HELLO %s" %values)
            return super().create_tracking_values(initial_value, new_value, col_name, col_info, track_sequence, model_name)
