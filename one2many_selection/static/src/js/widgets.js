odoo.define('web_one2many_selectable_10.form_widgets', function (require) {
	...
	ListRenderer.include({
		...
		_updateSelection: function () {
	        ...
	        if(this.selection.length > 0){
	        	$('.button_archive_hr_applicant').show()
                $('.button_assign_hr_applicant').show()
	        }else{
	        	$('.button_archive_hr_applicant').hide()
                $('.button_assign_hr_applicant').hide()
	        }
	...
	var One2ManySelectable = FieldOne2Many.extend({
		...
		events: {
			"click .button_archive_hr_applicant": "action_archive_hr_applicant",
            "click .button_assign_hr_applicant": "action_assign_hr_applicant",
		},
		start: function()
	    {
		...
		action_archive_hr_applicant: function()
		{		
			var self=this;
			...
			rpc.query({
                'model': 'hr.applicant.duplicate.wizard',
                'method': 'archive_hr_applicant',
                'args': [selected_ids],
            }).then(function(result){
                self.trigger_up('reload');
            });
		},
		action_assign_hr_applicant: function()
		{		
			var self=this;
			var selected_ids = self.get_selected_ids_one2many();
            /*var recruteur = self.data('user_id');*/
            var recruteur = parseInt($("input[name='user_id']").val());
			if (selected_ids.length === 0)
			{
				this.do_warn(_t("You must choose at least one record."));
				return false;
			...
			rpc.query({
                'model': 'hr.applicant.duplicate.wizard',
                'method': 'assign_hr_applicant',
                'args': [selected_ids, recruteur],
            }).then(function(result){
                self.trigger_up('reload');
            });
		},
